...
namespace :import do
  ...
  task :users, [:users] => :environment do
    User.create(email: 'admin@example.com', first_name: 'Admin',
                last_name: 'Adminington', password: ENV['ADMIN_PASSWORD'],
                role: 1)
  end

  desc :"Create Tutorials"
  task :tutorials, [:tutorials] => :environment do
    response = Faraday.get("https://www.googleapis.com/youtube/v3/playlists?
      key=#{ENV['YOUTUBE_API_KEY']}&
      part=snippet&channelId=UC2zYYOtckevoWTGDu5SdCkg&maxResults=50")
  ...
  task :videos, [:videos] => :environment do
    ...
    tutorials.each do |tutorial|
      response = Faraday.get("https://www.googleapis.com/youtube/v3/
        playlistItems?key=#{ENV['YOUTUBE_API_KEY']}&
